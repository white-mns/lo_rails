class PgwsController < ApplicationController
  ...
  def index
    placeholder_set
    param_set
    @count	= Pgw.notnil().includes(:p_name, :pgws_name).ransack(params[:q]).result.count()
    @search	= Pgw.notnil().includes(:p_name, :pgws_name).page(params[:page]).ransack(params[:q])
  ...
  def graph
    param_set
    @search	= Pgw.notnil().includes([:p_name, :pgws_name]).ransack(params[:q])
  end
