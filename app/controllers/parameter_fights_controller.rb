class ParameterFightsController < ApplicationController
  ...
  def index
    placeholder_set
    param_set
    @count	= ParameterFight.notnil().includes(:p_name).ransack(params[:q]).result.count()
    @search	= ParameterFight.notnil().includes(:p_name).page(params[:page]).ransack(params[:q])
